...
export function normalizeConnectionInput(
  ...
  return {
    ...
    sshEnabled: input.sshEnabled ?? false,
    sshHost: input.sshHost?.trim() || "",
    sshPort: input.sshPort ?? 22,
    sshUsername: input.sshUsername?.trim() || "",
    sshAuthMode: input.sshAuthMode ?? "private-key",
    sshPrivateKeyPath: input.sshPrivateKeyPath?.trim() || "",
    sshAgentPath: input.sshAgentPath?.trim() || "",

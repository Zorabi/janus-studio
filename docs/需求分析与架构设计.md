...
- 本地数据：SQLite
- 凭据保护：macOS 使用当前用户专属的 AES-256-GCM 本地凭据库；Windows/Linux 使用 Electron `safeStorage` 并支持本地加密回退
...
| 拓扑 | Label 自动分色、物理力导向、层级/环形/网格、节点与曲线拖动、画布平移缩放、全屏、详情、搜索、图例、布局持久化、PNG/JPG/SVG/JSON 导出 |
| Schema | Vertex Label、Edge Label、Property Key、Graph Index 结构化查看；C/M 索引随属性创建；索引生命周期；Schema 快照差异 |
| 数据迁移 | Janus Studio v1 整图 JSON，预览、可配置批次、批后停止、失败继续与日志、来源 ID 冲突策略；200 MB 本地文件保护 |
...
| DQ-01 | 数据质量 | Schema 感知的质量规则集与只读检查 | P1 |
| DQ-02 | 数据质量 | 有界/全量执行、问题样本、历史与完整审计导出 | P1 |
...
- `App.tsx` 仅负责应用壳、跨页面状态和页面调度
- `features/*` 按 query、connections、history、schema、transfer、settings 隔离页面与业务交互
- `components/ui` 存放无业务含义的共享弹窗、按钮、空状态和页面标题
- `lib` 存放持久化、格式化、键盘映射、结果建模等纯逻辑
- `styles/*` 按职责拆分，并通过单一入口保持明确且稳定的级联顺序
- Monaco Provider
...
SQLite 仅保存元数据与小型缓存。macOS 密码由当前用户目录中权限为 `0600` 的随机密钥进行 AES-256-GCM 加密，且不初始化系统钥匙串；Windows/Linux 优先由 `safeStorage` 产生密文，不可用时使用相同的本地加密回退。已解密密码仅保存在主进程会话内存缓存中，不进入 Renderer。
...
- macOS 禁用 Electron Cookie Encryption Fuse，凭据模块不加载 `safeStorage`，避免 Chromium 或应用逻辑重复访问 Keychain。
- Windows/Linux 按需延迟加载 `safeStorage`，不可用时自动回退到 AES-256-GCM 本地凭据库。
...
- Janus Studio v1 中小规模整图导入导出与查询结果导出。
...
- ConfiguredGraphFactory 动态多图、模板配置与 Graph Configuration 生命周期管理。
- SSO/Token/自定义认证。
- SSH Tunnel、代理和 mTLS。
- Schema Diff 与迁移脚本。
- 外部批处理作业集成。
...
已完成的增量能力：数据质量规则集、Schema 候选、快速/全量只读检查、问题样本、检查历史和完整审计导出。
...
- 每次执行自动写历史。
- 可搜索、筛选、重新打开和删除。
- 可导出 JSON、JSONL、CSV；整图使用 Janus Studio v1 JSON 归档。
- 导出失败不会留下被误认为完整的目标文件。
...
- 数据质量检查只执行只读 Gremlin，不提供自动修复、批量删除或隐式 Schema 变更。
- 每个规则集固定绑定 `connectionId + graphName + graphAccess`，切换页面或当前连接不能改变已保存规则和运行任务的目标。
- 支持孤立顶点、重复候选、必填属性、属性域、边端点、度数范围和分布概览七类规则；Vertex Label、Edge Label 和 Property Key 优先从当前 Schema 获取，同时允许输入过滤已有候选。
- 有界快速检查用于日常反馈，并明确显示扫描上限；全量检查作为主进程长任务运行，不能依赖 Renderer 页面是否挂载。

### 21.2 结果、样本与导出

- 结果先展示规则状态、问题数量和必要摘要。只读 Gremlin、完整样本属性和分布明细默认折叠，由用户按需展开。
- 问题样本摘要只展示最有辨识度的字段；当前约定为 `cp1`、`cp2`，完整属性在详情、查询标签页和导出中保留。
- 样本支持单条详情、整组展开/收回和“在查询中打开”，避免用户抄写 ID 后自行构造查询。
- CSV 面向业务表格，JSONL 面向流式处理，JSON 面向系统交换，审计报告面向非技术人员阅读；所有格式都必须覆盖全部问题样本，不能只导出 UI 当前页或前 50 条。
- 规则集和检查历史按每页 20 条分页；历史终态记录最多保留最近 200 条。规则集删除要求输入完整名称，检查历史支持显式删除。

### 21.3 UI 视觉验收基线

UI 验收不是主观抽查，而是每次页面修改的完成条件：

| 检查项 | 强制标准 |
|---|---|
| 主题控件 | `input`、`textarea`、选择器和数字框显式使用主题变量；深色主题无白底，浅色主题无低对比度文字 |
| 尺寸对齐 | 常规控件最小高度 44px；文字、Lucide 图标、计数和操作按钮垂直居中，同组控件高度一致 |
| 间距 | 字段、卡片、区块与弹窗头尾使用统一间距尺度；内容不得贴底、贴边或依靠负边距修补 |
| 滚动 | 一个区域只有一个明确滚动容器；设置 `min-height: 0`，固定头尾不遮挡最后一项 |
| 响应式 | 至少验证 1280×720 与窄窗口；无文字覆盖、横向越界、弹层裁切、按钮不可点击 |
| 信息密度 | 长文本可换行、展开或滚动；高密度详情默认收起，不直接铺满页面 |
| 交互状态 | Hover/Focus/Selected/Error/Disabled 只作用于当前目标，子按钮不穿透卡片，焦点环清晰可见 |
| 列表导航 | 超过单页容量必须显示总数、当前范围和分页；删除等危险操作遵循统一二次确认 |

代码实现应优先复用共享控件与现有设计 Token，不得通过裸 `input`、浏览器默认菜单或一次性颜色值建立平行视觉体系。

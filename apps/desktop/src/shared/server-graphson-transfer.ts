...
export type ConfiguredGraphTarget = { name: string; traversalSource: string };
export type DeletedVertexBatch = { deleted: number; complete: boolean };
export type VertexCount = { total: number };
export type ExportProgress = { exists: boolean; sizeBytes: number };
...
function decodeGraphValue(value: unknown): unknown {
  ...
  if (object["@type"] === "g:Map" && Array.isArray(object["@value"])) {
    ...
    for (let index = 0; index + 1 < values.length; index += 2) {
      decoded[String(decodeGraphValue(values[index]))] = decodeGraphValue(values[index + 1]);
    }
    return decoded;
  }
  if (typeof object["@type"] === "string" && "@value" in object) return decodeGraphValue(object["@value"]);
  return Object.fromEntries(Object.entries(object).map(([key, entry]) => [key, decodeGraphValue(entry)]));
}

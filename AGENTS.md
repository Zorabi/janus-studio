...
- 所有 `input`、`textarea`、选择器、搜索框和数字输入框必须显式使用 Janus Studio 主题变量，禁止依赖浏览器默认外观。深色主题不得出现白底、浅灰字或原生高亮；浅色主题也必须保证边框、文字、占位符和禁用态对比度。
- 表单控件默认最小高度为 44px；同一行控件的高度、圆角、边框、标签基线与图标垂直位置必须一致。仅在明确标注的紧凑工具栏中允许更小尺寸。
- 页面、卡片、弹窗、分组和操作区使用现有间距尺度。相邻字段不得贴边，内容区与弹窗头尾至少保留一个完整间距单位；禁止依靠偶然的 `margin` 抵消实现对齐。
- 可变高度页面或弹窗必须指定唯一滚动容器，并正确设置 `min-height: 0` 与 `overflow: auto`。固定头部、标签栏和操作栏不能遮挡内容，也不能让底部按钮覆盖最后一项。
- 新增或修改页面必须至少检查深色/浅色主题、1280×720 主窗口和窄窗口；检查文字覆盖、横向溢出、下拉层裁切、滚动可达、按钮可点击及焦点可见性。
- 列表超过单页容量时必须显示总数、当前范围和分页控件。长文本应换行、展开或进入可滚动详情，不能只留下无法区分的省略值。
- Hover、Focus、Selected、Error 和 Disabled 状态必须只作用于当前控件；禁止父级 hover 导致同组全部删除按钮或图标一起高亮，禁止点击子控件穿透触发卡片动作。
- 复杂信息默认使用摘要与渐进展开。Gremlin、错误详情、样本属性等高密度内容不得默认全部铺开，也不能要求用户自行复制 ID 后再次查询。
...
- 连接工作区支持安全导入导出 `janus-studio.connections/v1`：归档不得包含密码、Token、认证 Profile 引用、敏感/自定义 Header 值、私钥口令或本机证书/私钥路径。导入前按新增、更新、相同跳过和同名冲突审阅；更新现有连接时必须保留本机加密凭据与路径，新建连接则提示补充凭据。
- 连接测试摘要通过 SQLite v17 持久化最近探测时间、通过/失败、失败阶段和耗时，不保存可能含服务端正文的错误消息。连接页可由用户显式按当前筛选结果顺序批量探测；禁止后台自动批量访问真实连接。
...
- 数据质量工作区支持按固定连接与图上下文保存规则集，并提供孤立顶点、重复候选、必填属性、属性域、边端点、度数范围和分布概览七类只读检查。
- 规则中的 Vertex Label、Edge Label 和 Property Key 使用当前 Schema 候选与输入匹配；多选项以独立 Token 编辑，删除只影响目标 Token，保存失败必须定位到具体规则和字段。
- 检查支持有界快速检查与全量长任务；任务、结果与错误由主进程持久化，切换页面不得改变目标图或丢失状态。
- 问题样本默认折叠，只显示必要摘要；当前摘要属性固定为 `cp1`、`cp2`，详情、查询和导出仍保留完整属性。样本可逐条或整组展开、收回，并可在查询标签页中读取完整元素。
- 导出支持 CSV、JSONL、JSON 和面向业务阅读的审计报告；导出必须覆盖全部问题样本，而不是只导出界面已加载的前 50 条。
- 规则集与检查历史均使用主进程分页，默认每页 20 条；历史最多保留最近 200 条终态记录。规则集删除必须输入完整名称确认，历史记录支持显式删除。
...
11. UI 修改提交前必须逐项检查：深色/浅色输入框底色、控件高度与垂直对齐、区块上下间距、唯一滚动容器、固定操作栏遮挡、窄窗换行与溢出、弹层裁切、分页可见性、Hover/Focus 隔离。任一项未检查不得宣称 UI 完成。
